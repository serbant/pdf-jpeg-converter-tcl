...
set options [list \
    ...
    [list "excludeBusted.arg"     ""        $arrPlatformUsg(excludeBustedUsg)] \
    [list "cleanExcludeConverted"        $arrCommonUsg(cleanExcludeConverted)] \
    [list "cleanExcludeBusted"              $arrCommonUsg(cleanExcludeBusted)] \
...
proc execGs { gsBinary myFile { debug 0 } { fDebug } fExcludeBusted } {
...
proc setDefaults {{confPath ""} {homePath ""} {platform ""}} {
	...
	set excludeBustedDef    [list "fuckedFileDef"       "$confPath\.fucked"]
	...
	return -code ok [concat $configDef $logFileDef $errorFileDef \
	    $excludeConvertedDef $excludeBustedDef \
...
proc setUsage {{confPath ""} {homePath ""} {platform ""}} {
	...
	set excludeBustedUsg [list "excludeBustedUsg" "file containing a list \
	...
	return -code ok [concat $configUsg $logFileUsg $errorFileUsg \
	    $excludeConvertedUsg $excludeBustedUsg \
    ...
    set cleanExcludeBustedUsg [list "cleanExcludeBustedUsg" "remove \
	...
	$cleanExcludeBustedUsg $maxSizeLogsUsg $logLevelUsg \
